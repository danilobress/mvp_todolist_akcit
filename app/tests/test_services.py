...
def task_service(mock_repo, mock_ai_client, mock_bg_repo_factory):
    return TaskService(repo=mock_repo, ai_client=mock_ai_client, bg_repo_factory=mock_bg_repo_factory)

@pytest.mark.asyncio
async def test_create_task_assigns_default_priority_and_adds_background_task(task_service, mock_repo):
    # Arrange
    ...
    # Assert
    mock_repo.create_task.assert_awaited_once_with(task_data, initial_priority=TaskPriority.MEDIA.value)
...
async def test_get_tasks_returns_list_of_tasks(task_service, mock_repo):
    # Arrange
    expected_list = [
        TaskResponse(id=1, title="Task 1", description=None, is_completed=False, priority=TaskPriority.MEDIA),
        TaskResponse(id=2, title="Task 2", description=None, is_completed=True, priority=TaskPriority.ALTA)
    ]
...
async def test_update_task_empty_payload_returns_current_state(task_service, mock_repo):
    ...
    expected_response = TaskResponse(
        id=task_id, title="Old Title", description=None, is_completed=False, priority=TaskPriority.MEDIA
    )
    mock_repo.get_task.return_value = expected_response
...
@pytest.mark.asyncio
async def test_fetch_and_update_priority_bg_updates_db_when_priority_changes(task_service, mock_ai_client, mock_bg_repo, mock_bg_repo_factory):
    # Arrange
    ...
    mock_bg_repo.update_task_priority.assert_awaited_once_with(1, TaskPriority.ALTA.value)

@pytest.mark.asyncio
async def test_fetch_and_update_priority_bg_skips_update_when_priority_is_media(task_service, mock_ai_client, mock_bg_repo, mock_bg_repo_factory):
    # Arrange
...
@pytest.mark.asyncio
async def test_fetch_and_update_priority_bg_handles_exception(task_service, mock_ai_client, caplog):
    # Arrange
    ...
    # Assert
    assert "Erro na background task de prioridade da Tarefa 1: Erro catastrófico forçado" in caplog.text
