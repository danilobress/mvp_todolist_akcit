...
class PriorityAdvisorClient:
    ...
    async def suggest_priority(
        ...
        if not self.api_key:
            logger.warning(
                f"LLM_API_KEY não configurada. Aplicando fallback silencioso '{TaskPriority.MEDIA.value}'."
            )
        ...
        try:
            ...
            if clean_answer in valid_priorities:
                ...
                logger.warning(
                    f"LLM não retornou a prioridade esperada.\nConteúdo limpo: '{clean_answer}'\nResposta completa da API: {data}\nFallback para '{TaskPriority.MEDIA.value}'."
                )
                return TaskPriority.MEDIA

        except httpx.TimeoutException as e:
            logger.warning(
                f"Timeout ao conectar no LLM ({e}). Fallback silencioso para '{TaskPriority.MEDIA.value}'."
            )
            return TaskPriority.MEDIA
        except httpx.HTTPStatusError as e:
            # Captura erros como 401 Unauthorized ou 403 Forbidden
            logger.warning(
                f"Erro HTTP do LLM ({e.response.status_code}). Fallback silencioso para '{TaskPriority.MEDIA.value}'."
            )
            return TaskPriority.MEDIA
        except httpx.RequestError as e:
            logger.warning(
                f"Falha de rede ao acessar LLM ({e}). Fallback silencioso para '{TaskPriority.MEDIA.value}'."
            )
            return TaskPriority.MEDIA
        except Exception as e:
            logger.error(
                f"Erro inesperado no PriorityAdvisor ({e}). Fallback silencioso para '{TaskPriority.MEDIA.value}'."
            )
            return TaskPriority.MEDIA
